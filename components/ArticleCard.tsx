...
export default function ArticleCard({ article, variant = "horizontal" }: Props) {
  ...
  if (variant === "large") {
    return (
      <Link href={href} className="block group h-full">
        <div className="relative w-full h-64 md:h-80 bg-gray-100 overflow-hidden rounded-md">
          {article.cover ? (
            <Image
              src={article.cover}
              alt={article.title}
  ...
  if (variant === "vertical") {
    return (
      <Link href={href} className="block group">
        <div className="relative w-full h-36 bg-gray-100 overflow-hidden rounded-md">
          {article.cover ? (
            <Image
              src={article.cover}
              alt={article.title}
  ...
  return (
    <Link href={href} className="flex gap-4 group py-4 border-b border-gray-100 last:border-0">
      {article.cover && (
        <div className="relative shrink-0 w-24 h-20 bg-gray-100 overflow-hidden rounded-md">
          <Image
            src={article.cover}
            alt={article.title}
            fill
